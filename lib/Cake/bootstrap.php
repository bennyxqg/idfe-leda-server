...
if (!defined('FULL_BASE_URL')) {
	...
	if (isset($httpHost)) {
		if($httpHost == 'mrj.uu.cc'){//美人记
			$httpHost = 'tjzs.uu.cc';
		}
        // 2019-03-05 *.iplayergames.cn 乐易域名是阿里云转发到 *.uu.cc，$_SERVER 中的 http_host 需要替换为
//        $aliyunRemoteAddr = Configure::read('aliyun_remote_addr');
//        $aliyunRemoteAddr = '120.76.199.114';
//        if ($aliyunRemoteAddr) {
//            if (isset($_SERVER['REMOTE_ADDR']) && $_SERVER['REMOTE_ADDR'] == $aliyunRemoteAddr  && stripos($httpHost, 'uu.cc', -5) ) {
//                $httpHost = str_replace('uu.cc', 'iplaygames.cn', $httpHost);
//            }
//        }
        if($httpHost == 'ahcq.uu.cc'){ // 暗黑传奇
            $httpHost = 'ahcq.iplaygames.cn';
        }
        if($httpHost == 'mob100.uu.cc'){ // 路人100
            $s = 's';
        }
