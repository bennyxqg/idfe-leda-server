 ...
 * @package       app.View.Helper
 */
class LyHelper extends BaseHelper
{
    public $site_id = 26;
    ...
    function topData(){
        //新闻
        $news = $this->getContentByCategoryIdAndSiteId($this->site_id, 46, 0, 6, 1);
        //公告
        $gong_gao = $this->getContentByCategoryIdAndSiteId($this->site_id, 48, 0, 6, 1);
        //活动
        $gong_luo = $this->getContentByCategoryIdAndSiteId($this->site_id, 52, 0, 6, 1);
        //获取轮转图
        ...
        $blocks=array();
        for($i=0;$i<count($site_block);$i++)
        {
        	$blocks[$site_block[$i]['Abbr']]=$site_block[$i];
        }

        return compact("gong_gao", "news", "gong_luo", "total", "img_index_1","img_index_2","blocks");
