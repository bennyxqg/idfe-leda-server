 ...
 */
class XhzcHelper extends BaseHelper
{
    public $site_id = 101;//37;
    ...
    function topData(){
        ...
        if(!($this->isAndroid()||$this->isIos())){
            $row=$GLOBALS['db']->query("select id,site_id,str1,str2,created,type from `datas` where site_id=$this->site_id and type=8");
        }else{
        	$row=$GLOBALS['db']->query("select id,site_id,str1,str2,created,type from `datas` where site_id=$this->site_id and type=0");
        }
        $data=array();
        foreach($row as $v){
        	$data[]=$v['datas'];
        ...
        if(!($this->isAndroid()||$this->isIos())){
        	$row2=$GLOBALS['db']->query("select id,site_id,str1,str2,created,type from `datas` where site_id=$this->site_id and type=1");
        }else{
        	$row2=$GLOBALS['db']->query("select id,site_id,str1,str2,created,type from `datas` where site_id=$this->site_id and type=4");
        }
        ...
        //查询微信公众号、qq、贴吧等数据
        $other_data = $GLOBALS['db']->query("select Abbr,content from block where site_id=$this->site_id and Abbr in ('weixin_account','baidu_tieba_url','qq_group','index_video_url','weixin_qrcode','weibo_url','game_download_qrcode','android_download_url','ios_download_url','qq_qun_ios','qq_qun_android','forum_url')");
        $other = array();
        if ($other_data) {
