 ...
 * 钢铁力量
 * 新闻分类没有特殊显示要求的分类
 * 关于新闻加hot图标：没有收到具体需求，默认按此规则-按照home字段（是否推荐）倒序、updated更新时间倒序排列，home为1的记录加hot
 */
 ...
 *
*/
class IronForceHelper extends BaseHelper {
    public $siteId;
    public $limit;
    ...
    public function getNews2($cid, $page = 0, $home = 0, $order = '', $fields = array()){
    	$data = array();
    	...
    	if (is_array($cid)) { // 如果是数组则返回所有分类的新闻
    		foreach ($cid as $value) {
    			if ($value) {
    				$cateid = (int)$value;
    				$news = $this->getNewsByCategoryIdAndSiteId($this->siteId, $cateid, $offest, 1000, $home, $order);
    				$data[$cateid] = $news;
    			}
    		}
    	} else {
    		if ($cid) {
    			$cid = (int)$cid;
    			$news = $this->getNewsByCategoryIdAndSiteId($this->siteId, $cid, $offest, 1000, $home, $order);
