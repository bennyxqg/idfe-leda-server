<div class="main3">
    <ul class="tags">
        ...
        <li>
            <a href="<?php echo GET_BLOCK('kefu');?>">
...
<footer>
    ...
    <p>本游戏适合18岁以上的玩家进入 <br> Copyright©2009-<span class="copy-year">2018</span> 深圳市创梦天地科技有限公司 <br/>
                版权所有 粤ICP备11018438号
    </p>
...
<div class="pop" style="display:none;">
    <div class="qrcode">
        ...
        <h3>官方微信公众号：ftxdgame</h3>
        <div>长按扫码关注，或保存相册，微信中使用扫一扫>相册>选择二维码图片</div>
    </div>
</div>
<div class="phone_box">
    <div class="phone">
...
$(function(){
    ...
    function os() {
        if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
            $('.main1 .downLoad').hide();
            $('.main1 .downLoad').eq(1).show();
            $('header .download').hide();
            $('header .download').eq(1).show();
            return 1;
        } else if (/(Android)/i.test(navigator.userAgent)) {
            $('.main1 .downLoad').hide();
            $('.main1 .downLoad').eq(0).show();
            $('header .download').hide();
            $('header .download').eq(0).show();
            return 2;
        } else {
            $('.main1 .downLoad').hide();
            $('.main1 .downLoad').eq(0).show();
