...
<div class="page_content">
	<div class="pr page_wrap">
        ...
        <div class="content">
        <?php /*BLOCK*/
        	$category_id=GET_CURRENT_NEWS_INFO('news_categories_id');
        	$map=array('资讯'=>'NEWS','攻略'=>'STRATEGY','活动'=>'ACTIVITY');
        	echo '<h2 class="pr page_tit">'.GET_CATEGORY_NAME($category_id).'<i>'.$map[GET_CATEGORY_NAME($category_id)].'</i><a class="pa back_home" href="//ddx.uu.cc/" title="返回官网首页">返回首页</a></h2>';
            echo '<h3 class="news_tit">'.GET_CURRENT_NEWS_INFO('title').'</h3>';
            echo '<div class="date">'.date('Y-m-d H:i',GET_CURRENT_NEWS_INFO('created')).'</div>';
            echo '<div class="news_con">'.GET_CURRENT_NEWS_INFO('content').'</div>';
        /*BLOCK*/ ?>
        </div>
    </div>
</div>
