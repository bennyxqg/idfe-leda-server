<?php 
$this->R->init(array('site_id' => $site_id, 'limit' => 5));
$banner = $this->R->getSingleBlock('inner_banner');
$mostClicks = $this->R->getMostClicksNews($category_id); // 点击数最多的记录id
$lastNews = $this->R->getLastNews($category_id); // 最新的记录id

$allCategories = $categories = $this->R->getNewsCategory(); // 分类
unset($categories[$this->R->specialCategoryId]);
$categoryIds = array_keys($categories);
$news = $this->R->getNews($category_id, $page); // 正常显示的新闻
?>
                ...
                <div class="cont_news">
                    ...
                    <div class="cont_news_detail">
                            ...
                            <?php if($news):?>
                                <?php foreach ($news as $data):?>
                                    <li>
                                        ...
                                        <span class="news_desc"><a href="<?php echo $this->Gbsg->getShowUrl($data['id']);?>">
                                             <?php echo $data['title'];?>
                                            <?php 
                                            // 需求：最新的和点击数最多的记录加hot
                        ...
                        <div class="pagination">
                            <?php  echo $this->R->pagination((int)$category_id,(int)$page);?>
                        </div>
                    </div>
                </div>
            </div>
