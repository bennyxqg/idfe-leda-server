...
<?php 
    ...
    $allCategories = $this->Dfw->getNewsCategory(); // 所有新闻分类
$categories = $allCategories;

unset($categories[$this->Dfw->specialCategoryId]); // 首页不显示 公告 分类
unset($categories[$this->Dfw->roleCateId]); // 首页不显示 人物大全 分类
unset($categories[$this->Dfw->propCateId]); // 首页不显示 道具大全 分类

$categoryIds = array_keys($categories);
$news = $this->Dfw->getNews($categoryIds); // 显示的新闻
ksort($categories);
ksort($news);
?>
    <div class="page">
        ...
        <div class="content">
                        ...
                        <?php if ($loopImg) : // 顶部轮播图片?>
                            <?php foreach ($loopImg as $key => $value):?>
                                <?php if ($value['big_link']) : // 有链接时?>
                                    <div class="swiper-slide"><a href="<?php echo $value['big_link']; ?>"><img src="/uploads/<?php echo $value['big_url']; ?>" alt="<?php echo $value['alt'];?>" title="<?php echo $value['title'];?>" width="100%" /></a></div>
                                <?php else: // 无链接?>
            ...
            <div class="contact">
                <div class="wx fl"><i></i>微信号：我爱大富翁OL</div>
                <div class="qq fr"><i></i>玩家QQ群：467276195</div>
            </div>
        </div>
        <div class="footer">
